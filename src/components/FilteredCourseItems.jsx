...
const FilteredCourseItems = ({course, handleDelete}) => {
  ...
  useEffect(() => {
    ...
    const fetchCourses = async () => {
        const token = localStorage.getItem('token');
        const response = await axios.get(`https://genuis.tech/api/student-courses/${user && user.user_id}`, {
          headers: {
  ...
  const openCoursePageHandler = (event) => {
    ...
    history(`/course-item/${course.id}`); 
}

const handleEnrollClick = () => {
  if (user.loggedIn) {
    axios.post(`https://genuis.tech/api/enroll-to-course/${course.id}`,null, {
      headers: {
        'Authorization': `Token ${token}`,
...
export default FilteredCourseItems;
